$(document).ready(() => {
    ...
    $(document).on('click', "button[data-nuevo]", function (event) {
        $("#modal").modal("toggle");
        $("#exampleModalLabel").text("Nuevo almacen");
    ...
    const load = async () => {
        ...
        $.each(response, function (i, element) {
            ...
            row += `<tr>
                        <td>${element.nombre}</td><td>${element.descripcion}</td><td>${checked}</td>
            ...
            $(document).on("click", "button[" + editar +"=" + element.id + "" + "]", function () {
                ...
                $.get(Api+"/Get/" + id).then((response) => {
                    ...
                    $("#modal").modal("toggle");
                    $("#exampleModalLabel").text("Editar Almacen");
                    let formValidation = Module.get("form-validation");
                    formValidation.InitValidation();
    ...
    $(document).on("submit", "#frm", function (event) {
        ...
        let formValidation = Module.get("form-validation");
        let catAlmacen = formValidation.GetDataForm("data-name");

        $("#modal").modal("toggle");

        $.post(Api, catAlmacen).then(() => {
            swal({
                title: "Exito!",
